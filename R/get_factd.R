...
get_factd <- function() {
  ...
  if (factd == 2) {
    ...
    output_str = glue::glue(
      "{most_entries_reg$artist} has the most entries ever in the hottest 100 yearly countdown.
  ...
  if (factd == 4) {
    ...
    output_str = glue::glue(
      "{most_entries_top10_reg$artist} has the most top 10 entries in the yearly countdowns.
  ...
  if (factd == 5) {
    ...
    output_str = glue::glue(
      "{num_countdowns$artist} has appeared in largest number of past countdowns.
  ...
  if (factd == 6) {
    ...
    output_str = glue::glue(
      "{num_countdowns_reg$artist} has appeared in {num_countdowns_reg$n} yearly countdowns."
    ) |>
      glue::glue_collapse() |>
