...
cache_file <- paste0(
  "./data-raw/cache/hottest100-scrape-",
...
hottest100_track_json <- substr(
  hottest100_script,
  track_i + attr(track_i, "match.length"),
  album_i - 1
)
hottest100_album_json <- substr(
  hottest100_script,
  album_i + attr(album_i, "match.length"),
  nchar(hottest100_script) - 1
)
